...
- `plans/2026-08-19-chrome-tokenization.zh.md` owns the pending refactor that moves the tool chrome's hardcoded colors into `--ui-*` tokens; it must not touch the `.site-*` Preview rules.
...
3. `Website tokens`: the generated system as purpose-named tokens for both themes, with `Custom pairs` (editable foreground/background snapshots) nested beneath it.
