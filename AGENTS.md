...
Two paths share one system. `Quick start` asks for a color character, a Brand source, and a Secondary strategy, then generates a complete accessible website color system in one action — no HEX value required. `Advanced editing` keeps the direct workflow: Background, Text, Brand, Neutral, optional Secondary, semantic roles, locks, scales, Role checks, and Custom pairs. Every result is inspected through WCAG contrast relationships, previewed as components, and exported.
...
- Product coverage profiles are additive component contracts, not color presets. They reuse the active Reference → Role → Website system, add profile relationships to the same validation summary, and never change color character or semantic hue identity.
...
- Step 03 also owns the product coverage selector. `General website` adds nothing; Dashboard, Marketing, Portfolio, and Documentation add their registered token groups, required/advisory relationship checks, and one matching Preview module. Changing a profile or an Advanced role re-resolves Preview, validation, CSS, JSON, and Tailwind from the same state owner.
