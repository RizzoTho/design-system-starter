...
- `js/token-contract.js` owns website token definitions, target profiles, relationship definitions, Light / Dark website-token resolution, and validation summaries.
...
10. Check desktop and narrow layouts. The scale may scroll horizontally, but pair fields and copy must remain usable, and Quick start must not overflow.
11. Report static checks and browser checks separately. Never claim visual verification if only source checks ran.
